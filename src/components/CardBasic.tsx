...
export const CardBasic = ({name,email,brind}:any) => {
  const router = useRouter();
  return (
    <div className="flex items-center justify-center mt-4 w-full">
    <div className="bg-white flex rounded-3xl border shadow-xl p-8 w-3/6">
      <div className="flex justify-between items-center">
        <button 
        onClick={()=>{
          router.push("/student/settings")
        }}
        className="inline-flex items-center justify-center w-14 h-14 text-blue-100 bg-gradient-to-r from-yellow-400 to-yellow-500 rounded-full">
      ...
      <div>
        <h1 className="font-semibold text-xl text-gray-700 ml-5">Основни подаци</h1>
        <h1 className="font-semibold text-xl text-gray-500 ml-5">Име и презиме: {" "+ name}</h1>
        <h1 className="font-semibold text-xl text-gray-500 ml-5">Е-mail адреса:{" "+ email}</h1>
        <h1 className="font-semibold text-xl text-gray-500 ml-5">Број индекса:{" "+ brind}</h1>
      </div>
    </div>
  </div>
  );
